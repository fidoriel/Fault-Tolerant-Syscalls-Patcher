...
static int do_register_kprobe(struct kprobe *kp, char *symbol_name,
  ...
  if (ret < 0) {
    pr_err("[sys_call_patcher:kln] do_register_kprobe: failed to register for "
           "symbol %s, returning %d\n",
           symbol_name, ret);
    return ret;
  }

  pr_info("[sys_call_patcher:kln] Planted krpobe for symbol %s at %p\n",
          symbol_name, kp->addr);
...
kln_p get_kln_p(void) {
  ...
  pr_info("[sys_call_patcher:kln] kallsyms_lookup_name address = 0x%lx\n",
          kln_addr);
